...
# ════════════════════════════════════════════════════════════
#  SHARED BACKUP PANEL BUILDER (used by all 3 systems)
# ════════════════════════════════════════════════════════════
_BACKUP_SETTINGS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                      "backup_settings.json")

def _load_backup_settings():
    try:
        with open(_BACKUP_SETTINGS_FILE) as f:
            return json.load(f)
    except Exception:
        return {"last_backup": "Never", "auto_sync": True}

def _save_backup_settings(d):
    try:
        with open(_BACKUP_SETTINGS_FILE, "w") as f:
            json.dump(d, f, indent=2)
    except Exception as e:
        print(f"Backup settings save error: {e}")

def _run_backup_to_firestore(db_module, offline_db_module):
    """Push all local SQLite data to Firestore."""
    import sqlite3
    conn = sqlite3.connect(str(offline_db_module.DB_PATH))
    ...
    for row in cur.fetchall():
        ...
        try:
            db_module._set_doc("transactions", txn_id, {
    ...
    for row in cur.fetchall():
        ...
        try:
            db_module._set_doc("transaction_items", doc_id, {
            ...
            print(f"Backup item {doc_id}: {e}")
    # Products
    cur.execute("SELECT * FROM products_cache")
    for row in cur.fetchall():
        d = dict(row)
        store = d.get("store","cafestore"); barcode = d.get("barcode","")
        try:
            db_module._set_doc("products", f"{store}_{barcode}", {
                "barcode": barcode, "name": d.get("name",""),
            ...
            print(f"Backup product {barcode}: {e}")
    conn.close()

def _run_restore_from_firestore(db_module, offline_db_module):
    """Pull data from Firestore → SQLite."""
    for store in ["cafestore", "canteen"]:
        prods = db_module.get_all_products(store=store)
        offline_db_module.save_products_cache(prods, store)
    for store in ["coop","cafestore","canteen"]:
        try:
            results = db_module._query("loyalty_members",
                                        filters=[["store","EQUAL",store]])
            members = []
            for r in results:
                d = db_module._parse_doc(r)
                members.append({
            ...
            offline_db_module.cache_loyalty_members(members, store)
        except Exception as e:
            print(f"Restore loyalty {store}: {e}")

def _build_backup_content(container, app_root, db_module, offline_db_module, datetime_cls):
    """Build the backup panel UI — clean natural theme matching the app."""
    import sqlite3
    bk_s = _load_backup_settings()

    # ── Header ──
    hdr = ctk.CTkFrame(container, fg_color="#1565C0", height=50, corner_radius=0)
    ...
    # ── Status card ──
    online = db_module.has_internet()
    st_f = ctk.CTkFrame(container, fg_color="#FFFFFF", corner_radius=8,
                         border_width=1,
                         border_color="#BBDEFB")
    ...
    last_lbl = ctk.CTkLabel(st_f,
                             text=f"Last: {bk_s.get('last_backup','Never')}",
                             font=ctk.CTkFont(size=9),
                             text_color="#546E7A")
    last_lbl.pack(side="right", padx=10)
    ...
    # ── Backup button ──
    def _do_backup():
        if not db_module.has_internet():
            messagebox.showwarning("Offline",
                "No internet.\nConnect and try again.", parent=app_root); return
        backup_btn.configure(text="⏳ Backing up...", state="disabled")
        container.update()
        def _bg():
            try:
                _run_backup_to_firestore(db_module, offline_db_module)
                dt = datetime_cls.now().strftime("%Y-%m-%d %H:%M:%S")
                bk_s["last_backup"] = dt
                _save_backup_settings(bk_s)
                app_root.after(0, lambda: [
                    backup_btn.configure(text="☁️  Backup Database", state="normal"),
                ...
                app_root.after(0, lambda: [
                    backup_btn.configure(text="☁️  Backup Database", state="normal"),
                    messagebox.showerror("❌ Error", str(e), parent=app_root)
                ])
        threading.Thread(target=_bg, daemon=True).start()

    backup_btn = ctk.CTkButton(container, text="☁️  Backup Database",
                                fg_color="#1565C0", hover_color="#1976D2",
                                command=_do_backup, **btn_kw)
    backup_btn.pack(fill="x", padx=12, pady=(0,6))
    ...
    def _do_restore():
        ...
        if not messagebox.askyesno("Restore",
            "⚠️ Replace local data with Firestore data?\n\nContinue?",
            parent=app_root): return
        restore_btn.configure(text="⏳ Restoring...", state="disabled")
        container.update()
        def _bg():
            try:
                _run_restore_from_firestore(db_module, offline_db_module)
                app_root.after(0, lambda: [
                    restore_btn.configure(text="🔄  Restore Database", state="normal"),
                    messagebox.showinfo("✅ Restored",
                        "Restored from Firestore!", parent=app_root)
                ])
            except Exception as e:
                app_root.after(0, lambda: [
                    restore_btn.configure(text="🔄  Restore Database", state="normal"),
    ...
    restore_btn = ctk.CTkButton(container, text="🔄  Restore Database",
                                 fg_color="#546E7A", hover_color="#37474F",
                                 command=_do_restore, **btn_kw)
    restore_btn.pack(fill="x", padx=12, pady=(0,6))

    # ── Divider ──
    ctk.CTkFrame(container, fg_color="#E0E0E0", height=1).pack(fill="x", padx=12, pady=10)

    # ── DB Stats ──
    ctk.CTkFrame(container, fg_color="#E0E0E0", height=1).pack(fill="x", padx=12, pady=6)
    stats_f = ctk.CTkFrame(container, fg_color="#F5F7FA", corner_radius=8,
                            border_width=1, border_color="#E0E0E0")
    stats_f.pack(fill="x", padx=12, pady=(0,12))
    ctk.CTkLabel(stats_f, text="🗄️ Local Database",
                 font=ctk.CTkFont(size=11, weight="bold"),
                 text_color="#546E7A").pack(anchor="w", padx=12, pady=(10,4))
    try:
        conn = sqlite3.connect(str(offline_db_module.DB_PATH))
        cur  = conn.cursor()
        ...
        conn.close()
        ctk.CTkLabel(stats_f, text=f"  Transactions: {t}",
                     font=ctk.CTkFont(size=11), text_color="#546E7A"
                     ).pack(anchor="w", padx=12, pady=1)
        ctk.CTkLabel(stats_f, text=f"  Products: {p}",
                     font=ctk.CTkFont(size=11), text_color="#546E7A"
                     ).pack(anchor="w", padx=12, pady=(1,10))
    except Exception as e:
        ctk.CTkLabel(stats_f, text=f"  DB error: {e}",
...
SHORTCUTS_FILE = os.path.join(_BASE_DIR, "shortcuts_config.json")
...
class StoreDashboard(ctk.CTkToplevel):
    ...
    def __init__(self, login_window, username):
        ...
        self.title("Cafeteria Store — Point of Sale")
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(center, text="CAFETERIA STORE",
                     font=ctk.CTkFont(family="Georgia", size=24, weight="bold"),
                     text_color="white").pack()
        ctk.CTkLabel(center, text="Store  |  Point of Sale",
    ...
    def _build_left(self, parent):
        ...
        # Auto-focus barcode on startup and after every transaction
    ...
    def _open_checkout_popup(self):
        ...
        _update_fields()
        # Auto-focus on open (default Cash + Student = cash entry)
        popup.after(100, _focus_correct_field)
        # Enter key confirms payment from anywhere in popup
        ...
        def _confirm():
            ...
            if pay == "Cash":
                ...
                if cat == "Others":
                    # Run in background so checkout doesn't freeze
    ...
    def _show_payment_success(self, change):
        ...
        # Must focus window to receive keyboard events
    ...
    def _save_and_clear(self, method, total, cash=0.0, change=0.0,
        ...
        try:
            ...
            db.save_transaction(dt, total, method, cash, change,
                                ...
                                buyer_type=buyer_type, store="cafestore")
        except Exception as e:
            print("Save error:", e)
    ...
    #  • Persistent: saved to shortcuts_config.json
    ...
    def _fkey_refresh(self):
        ...
        def _do():
            try:
                ...
                db.get_all_products(store="cafestore")
                msg = "✅ Refreshed!"
                if synced > 0:
...
if __name__ == "__main__":
    ...
    def _after_splash():
        root = ctk.CTk()
        root.withdraw()
        StoreDashboard(root, "store").mainloop()
